...
void Heap<X>::percolate_down(u_int curr)
{
    if(is_min_heap())
    {
        ...
        while(left(curr) < arr.size() && DEREF arr[curr] > DEREF arr[min_child(curr)]) 
        {
    ...
    {
        u_int max_c;
        while(left(curr) < arr.size() && DEREF arr[curr] < DEREF arr[max_child(curr)]) 
        {
...
void Heap<X>::percolate_up(u_int curr)
{
    if(is_min_heap())
    {
        ...
        while(curr > 0 && DEREF arr[curr] < DEREF arr[par(curr)]) 
        {
