...
class List 
{
    ...
    public:
        ...
        void reverse(void)
        {
            ...
            Iterator<T> fin = end();
            for(int i = 0; i <= floor(listLen / 2); ++i)
            {
        ...
        const T operator[] (const u_int n) const
        {
             ...
             *      if(1 < 3)
             */
            if(((listLen - 1) - n) < (u_int)floor((listLen / 2)))
            {
                // Start at the "tail"
                curr = access;
                // Work backwards
        ...
        void overwrite(u_int index, T foo)
        {
            ...
            if(((listLen - 1) - index) < (u_int)floor((listLen / 2)))
            {
                curr = access;
 ...
 */
template<typename T>
inline ostream& operator<< (ostream &ost, List::List<T> &list)
{
