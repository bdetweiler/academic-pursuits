...
class Matrix : public brd::cVector<brd::cVector<VAL_TYPE> >
{
	public:
        ...
        void init(const VAL_TYPE& val)
        {
            ...
            // TODO: This is only for string type
            colList.push_back("");
            rowList.push_back("");
        ...
        Matrix<VAL_TYPE, TITLE_TYPE>& I(void)
        {
            for(size_type i = 0; i < rows; ++i)
                for(size_type j = 0; j < cols; ++j)
                    operator[](i)[j] = (i == j) ? VAL_TYPE(i) : VAL_TYPE(0);
            return *this;
        }
     ...
     *              0 0 0 0 0 0 0 0 20 
     */
        void insert_at(size_type r, size_type c, VAL_TYPE val, 
                       TITLE_TYPE rTitle = "", TITLE_TYPE cTitle = "")
        {
            ...
            {
                // If the the size of the row title exists in printCellSize
                // we need to erase it because it is about to change.
            ...
            {
                // If the the size of the column title exists in printCellSize
                // we need to erase it because it is about to change.
            ...
            printCellSize.push_back(numToString(val).size());
            printCellSize.sort();
...
cout << "rowList.size() = " << rowList.size() << endl;
...
cout << "rowList.overwrite(" << r << ", " << title << ")" << endl;
