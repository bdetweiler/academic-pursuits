...
{
    regex_t preg[BUF_LEN];

    regcomp(preg, VALID_TEST, REG_ICASE);
    ...
    if(file)
    {
        ...
        for(u_int time = 0; time < time_units; ++time)
        {
            ...
            if(time == 0 || pushedOnHeap)
            {
                ...
                if(regexec(preg, buf, nmatch, pmatch, REG_NOTEOL) != REG_NOMATCH)
                {
                    ...
                    // programName
                    // Make sure to trim it up all purrty before we assign it.
                    programName = trim(strtok(NULL, "\0"));
                }
                // Reset the heap flag
                pushedOnHeap = false;
            }
