 ...
 * Notes:   Builds a hash table using a vector<hList*>
 */
Hash::Hash(void)
{ 
    debug = false;
    ...
    // Have to set up our table by creating the vectors first
    for(u_int i = 0; i < TABLE_SIZE; ++i)
        table.push_back(new vector<Pair<string, u_int>*>());
}
 ...
 * return:  vector<hList*>
 * time:    O(1)
 * notes:   Returns the actual table which is a vector of hlist pointers.
 */
vector<vector<hList*>*> Hash::getTable(void)
{
    return table;
}
 ...
 *          vector<hList*> for the right soundex. Thankfully, our hash 
 ...
 * amort:   O(m) - lookup is O(1) amortized time, so the hlist function
 ...
 * notes:   Returns the hlist of words for a soundex key, or a blank
 *          hList if the spot is empty.
 */
hList* Hash::lookup(char* key)
{
    hList* rval = NULL;
    ...
    // (*table[pos]) gives us a vector<hList*>
    // There is always a hlist in each slot. If the first hlist is empty though,
    ...
    if((*(*table[pos])[0]).size() > 0)
    {
        ...
        for(u_int i = 0; i < (*table[pos]).size() && !rval; ++i)
        {
            ...
            // If the soundex keys match, we'll return this hlist
            if(strncmp((*(*table[pos])[i]).getSoundex(), key, SOUNDEX_LEN) == 0)
                rval = ((*table[pos])[i]);
        }

        // If we didn't find a matching soundex, 
        // we'll push back a new hlist and return that
        if(!rval)
        {
            ...
            ((*table[pos]).push_back(new hList()));
            rval = (*table[pos]).back();
        }
    }
    // There's only one hlist.
    // If that one hlist is not empty
    else if(!((*(*table[pos])[0]).empty()))
    {
        if(debug)
            cout << "getSoundex() = " << (*(*table[pos])[0]).getSoundex() << endl;
        // If the soundex matches, we'll return this hlist
        if(strncmp((*(*table[pos])[0]).getSoundex(), key, SOUNDEX_LEN) == 0)
            rval = (*table[pos]).back();
        // If not, we need to push back another hlist and return that
        else
        {
            if(debug)
                cout << "COLLISION @ " << pos << " on key " << key << endl;
    ...
    // It's empty. Return the only hlist in the vector
    else
        rval = (*table[pos]).back();
