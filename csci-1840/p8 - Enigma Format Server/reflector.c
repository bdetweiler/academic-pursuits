 ...
 *         corresponding functions to make them class 
 *         methods.
 */
void reflector_init(Reflector* r, char refType)
{
    TRACEF(BUG) printf("reflector.c:"
                       " relector_init():"
                       " START\n");
...
#if 0
    /* In case we decide to add the 
     * other two reflector types
     */
    else if(refType == 'D')
        strncpy(r->alphaRing, 
                    REFLECTOR_B_THIN, 
    ...
    {
        fprintf(stderr, "reflector.c:"
                        " reflector_init:"
                        " Incorrect reflector assignment\n");
        exit(EXIT_FAILURE);
    }
