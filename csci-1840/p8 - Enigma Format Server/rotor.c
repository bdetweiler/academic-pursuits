 ...
 */
void rotor_init(Rotor* r, const int rotorNum, 
                const char offset, const char ringSetting,
                const int location)
{
    /* 
    ...
    /* 
     * This is the "key", the starting point for the 
     * alphaRing. We take the given char and look up 
     * its position on the standard alphabet side. 
     * A = 0, B = 1, etc. 
     * It doesn't change.
     */
    // r->ringSetting  = findCharNum(offset); // old
    if(ringSetting)
        r->ringSetting = findCharNum(ringSetting);
    else
        error("rotor", "rotor_init", "ringSetting was null");
        // r->ringSetting = 0;
     ...
     */
    // r->offset = r->ringSetting; // old
    if(offset)
        r->offset = findCharNum(offset);
    else
        r->offset = 0;
    ...
    /*
     * This will tell us which physical rotor we are
     * using. I, II, III, IV, or V, represented as
     * 1, 2, 3, 4, or 5.
     */
    r->rotorNum     = rotorNum;
    ...
    if(rotorNum == 1)
    {
        strncpy(r->alphaRing,    
                    ROTOR_I,       
                        sizeof(r->alphaRing));
        strncpy(r->alphaRingRev, 
                    ROTOR_I_REV,   
                        sizeof(r->alphaRingRev));
        strncpy(r->knock,        
                    NOTCH_I,       
                        sizeof(r->knock));
    } 
    ...
    else if(rotorNum == 5)
    {
        ...
        error("rotor", "rotor_init", 
             "Incorrect rotor assignment");
...
{
    // r->ringSetting  = findCharNum(ch); // old
    r->offset       = findCharNum(ch); // r->ringSetting;
}
...
char rotorSub(Rotor* r, const char ch)
{
     ...
     * rotor's offset.
     */
    tmp = (tmp - r->offset + r->ringSetting);
...
char rotorRevSub(Rotor* r, const char ch)
{
     ...
     */
    tmp  = ((tmp - r->offset + r->ringSetting) + 26) % 26;
    return tmp + 'A';
}
 ...
 *         Note: To double step or anything else, it will 
 *         have to be handled in the Spindle.
 */
void rotorStep(Rotor* r)
{
    ...
    else if(r->offset == 25)
    {
        ...
        error("rotor", "rotorStep", 
              "Illegal rotor movement.");
}
 ...
 * Notes:  Takes an alphabet character and returns what 
 *         position it is numerically (starting with 0). 
 *         We can use this number to find the corresponding 
 *         alpha character in the rotor array.
 */
int findCharNum(const char ch)
{
    // If it is not an alpha character
    if(!ch || (toupper(ch) < 'A' || toupper(ch) > 'Z'))
        error("rotor", "findCharNum", 
              "Not an alpha character.");
    /* As long as there is a valid character, 
     * it will return an int between 0 and 25.
     */
...
/* error
 * --
 * Input:  const char*, const char*, const char*
 * Output: Error messages to stderr
 * Return: None
 * Notes:  This is to keep all the error messages
 *         in one place so I don't have to worry 
 *         about minute details such as newlines
 *         and shit.
 */
