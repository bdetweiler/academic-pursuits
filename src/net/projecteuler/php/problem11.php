...
$arr = array(8, 2,  22, 97, 38, 15,  0, 40,  0, 75,  4,  5,  7, 78, 52, 12, 50, 77, 91,  8,  // 0
...
for($i = 0; $i < count($arr); ++$i) {
    ...
    $curDiagLeft1  = $cur + 20 - 1;
    $curDiagLeft2  = $cur + 40 - 2;
    $curDiagLeft3  = $cur + 60 - 3;

    $curDiagRight1 = $cur + 20 + 1;
    $curDiagRight2 = $cur + 40 + 2;
    $curDiagRight3 = $cur + 60 + 3;
    ...
    if($curRight1 % 20 != 0 &&
       ...
       $curRight3 < count($arr)) {
        ...
        $forward = $arr[$cur] * $arr[$curRight1] * $arr[$curRight2] * $arr[$curRight3];
        // Check forward
        if($forward > $max) {
            $max = $forward;
        }
    ...
    if($curDown3 < count($arr)) {
        $down = $arr[$cur] * $arr[$curDown1] * $arr[$curDown2] * $arr[$curDown3];
        if($down > $max) {
            $max = $down;
        }
    }

    if($curDiagLeft3 % 20 != 0       &&
       ($curDiagLeft2 - 1) % 20 != 0 &&
       ($curDiagLeft1 - 2) % 20 != 0 &&
       ($cur - 3) % 20 != 0) {

        $diagLeft = $arr[$cur] * $arr[$curDiagLeft1] * $arr[$curDiagLeft2] * $arr[$curDiagLeft3];
        if($diagLeft > $max) {
            $max = $diagLeft;
        }
    }

    if($curDiagRight3 % 20 != 0       &&
       ($curDiagRight2 + 1) % 20 != 0 &&
       ($curDiagRight1 + 2) % 20 != 0 &&
       ($cur + 3) % 20 != 0) {

        $diagRight = $arr[$cur] * $arr[$curDiagLeft1] * $arr[$curDiagLeft2] * $arr[$curDiagLeft3];
        if($diagRight > $max) {
            $max = $diagRight;
        }
    }
